...
import {
  ...
  pendingTripData, columnHeaders, filteredData, newFormedData, listOfDataObj, routeAnalyticsValue
...
describe('DataFromDB', () => {
  ...
  describe('getRoutes', () => {
    it('should return routes fetched from the db', async () => {
      const routeSpy = jest.spyOn(routeBatchService, 'getRoutes')
  ...
  describe('getRouteAnalysis', () => {
    it('should return routes analysis fetched from the db', async () => {
      const routeSpy = jest.spyOn(RouteUseRecordService, 'getRouteTripRecords')
        .mockReturnValue({ route: routesMock });
      const flatter = jest.spyOn(DataFromDB, 'flattenRoutesAnalyticsObject').mockResolvedValue();
      const result = await dataFromDB.getRouteAnalysis();
      expect(routeSpy).toHaveBeenCalledTimes(1);
    ...
    it('Should return flatten Routes Analytics Object', () => {
      const func = DataFromDB.flattenRoutesAnalyticsObject(routeAnalyticsValue);
      expect(func).toBeDefined();
      expect(func).toEqual(routeAnalyticsValue);
    });
  });
