...
import {
  getSlackDateTime, getSlackDateString, getSlackDateTimeString, timeTo12hrs
} from '../dateHelpers';
...
describe('dateHelpers', () => {
  ...
  describe('getSlackDateTimeString', () => {
    beforeAll(() => {
      result = getSlackDateTimeString(testDate);
    });

    it('should return expected string', () => {
      const date = moment(testDate).format('dddd, MMMM Do YYYY');
      const time = moment(testDate).format('h:mm a');
