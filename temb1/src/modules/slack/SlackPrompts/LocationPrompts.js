...
class LocationPrompts {
  ...
  static sendLocationCoordinatesNotFound(respond) {
    ...
    attachment.addFieldsOrActions('actions', [
      new SlackButtonAction('no', 'Enter Google Plus Code', 'no')
    ]);
  ...
  static sendDatbaseLocationSuggestionResponse(SuggestedLocations) {
    ...
    attachment.addFieldsOrActions('actions', [
      SuggestedLocations.length
        ? new SlackSelectAction('DatabaseSuggestions', 'Select Home location', SuggestedLocations)
        : new SlackButtonAction('retry', 'Try Again', 'retry'),
      new SlackButtonAction('not_listed', 'Location not listed', 'no')]);
