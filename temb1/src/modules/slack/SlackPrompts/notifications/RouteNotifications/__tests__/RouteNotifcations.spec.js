...
describe('Route Notifications', () => {
  ...
  describe('sendRouteUseConfirmationNotificationToRider', () => {
    ...
    it('should send route update notification to all riders', async () => {
      ...
      const routeServiceSpy = jest.spyOn(routeBatchService, 'getRouteBatchByPk')
  ...
  describe('sendRouteTripReminder', () => {
    ...
    it('should send whatsapp route trip reminder to driver', async (done) => {
      const driverMessage = {
        to: data.driver.driverPhoneNo,
        body: `Hello *${data.driver.driverName}*, You have an upcoming trip on route
        *${routeBatch.route.name}*\n\n
        We wish you a safe trip.`,
      };

      jest.spyOn(RouteNotifications, 'getRouteTripReminderMessage').mockReturnValue(driverMessage);
      jest.spyOn(WhatsappService, 'send').mockResolvedValue(null);
      await RouteNotifications.sendWhatsappRouteTripReminder(
        data.driver,
        routeBatch,
      );
      expect(RouteNotifications.getRouteTripReminderMessage).toHaveBeenCalled();
      expect(WhatsappService.send).toHaveBeenCalledWith(driverMessage);
      done();
    });
    ...
    it('should get whatsapp route trip reminder message', () => {
      const messageDetails = {
        ...
        + '*Batch*: undefined\n'
        + `*Takes Off At*: ${routeBatch.takeOff}\n`
        + `*Cab Reg No*: ${routeBatch.cabDetails.regNumber}\n\n`
        + 'We wish you a safe trip.',
      };
      const sendMessage = RouteNotifications.getRouteTripReminderMessage(data.driver, routeBatch);
