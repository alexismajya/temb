...
export class MailTemplate {
  static getProviderTripNotificationTemplate(options: NotificationTemplate) {
    const { name, url, tripDetails, timezone } = options;
    const {
      origin: { address: pickup },
      destination: { address: destination },
      rider: { name: passenger, phoneNo: passengerPhone },
      departureTime,
      noOfPassengers,
    } = tripDetails;
                   ...
                   <ul style="${style.ul}">
                       ...
                       <li style="${style.li}"><div style="${style.firstItem}">TakeOff Time</div>
                        <div style="${style.secondItem}">
                          ${moment(departureTime, moment.ISO_8601).tz(timezone).format('MMMM Do, YYYY HH:mm')}
                        </div>
                       </li>
                   </ul>
                 </div>
