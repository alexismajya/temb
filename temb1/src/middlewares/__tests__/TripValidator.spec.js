...
describe('Trip Validator', () => {
  ...
  describe('validateEachInput method', () => {
    ...
    it('should call validateAll with invalid driverPhoneNo ', async (done) => {
      const testReq = { ...req };
      testReq.body.driverPhoneNo = 'sdd';
      HttpError.sendErrorResponse = jest.fn(() => { });
      jest.spyOn(tripService, 'checkExistence').mockResolvedValue(true);

      await TripValidator.validateAll(testReq, res, next);
    ...
    it('should call validateAll with invalid tripId ', async (done) => {
      HttpError.sendErrorResponse = jest.fn(() => { });
