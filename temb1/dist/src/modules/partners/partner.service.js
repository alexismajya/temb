"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const sequelize_1 = require("sequelize");
const partner_1 = __importDefault(require("../../database/models/partner"));
const base_service_1 = require("../shared/base.service");
const database_1 = __importDefault(require("../../database"));
class PartnerService extends base_service_1.BaseService {
    constructor(partner = database_1.default.getRepository(partner_1.default)) {
        super(partner);
    }
    findOrCreatePartner(name) {
        return __awaiter(this, void 0, void 0, function* () {
            const [partner] = yield this.model.findOrCreate({
                where: { name: { [sequelize_1.Op.iLike]: `${name}%` } },
                defaults: { name },
            });
            return partner;
        });
    }
}
exports.partnerService = new PartnerService();
exports.default = PartnerService;
//# sourceMappingURL=partner.service.js.map