...
export class TripRequestService {
  ...
  constructor(private http: HttpClient, public toastr: AlertService) {
    ...
    private providerConfirmUrl = `${environment.tembeaBackEndUrl}/api/v2/provider/confirm`;
  ...
  providerConfirm(confirmData: Object): Observable<any> {
    return this.http.post<any>(this.providerConfirmUrl, confirmData);
  }
}
