...
export class ProviderInventoryComponent implements OnInit, OnDestroy {
  ...
  getSearchResults = async searchItem => {
    if (searchItem) {
      ...
      this.providers = this.providers.filter(provider =>
        provider.name.toLowerCase().includes(searchItem.toLowerCase())
      );
  ...
  getProvidersData = () => {
    this.isLoading = true;
    this.providerService.getProviders(this.pageSize, this.pageNo).subscribe(
      providerData => {
        const {
          providers,
        ...
        this.appEventsService.broadcast({
          ...
          content: { badgeSize: this.totalItems, actionButton: 'Add Provider' }
        });
        this.isLoading = false;
      },
      () => {
